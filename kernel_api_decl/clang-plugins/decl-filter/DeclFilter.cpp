...
static llvm::StringRef SimplifyPath(llvm::StringRef path) {
	if (path.find("/../") == llvm::StringRef::npos)
		return path;
	while (path.find("/../") != llvm::StringRef::npos) {
		int pos_dot = path.find("/../");
		int pos_delete = path.rfind('/', pos_dot - 1);
		std::string left = path.substr(0, pos_delete + 1).str();
		std::string right = path.substr(pos_dot + 4, path.size() - pos_dot - 4).str();
		path = llvm::StringRef(left + right);
	}
	return path;
}
